...
// Rule is a serializable representation of one robots rule. Pattern holds the
// compiled wildcard expression source when the path contains wildcards.
type Rule struct {
	Path    string
	Pattern string
	Allow   bool
...
// Rules returns the derived rules that apply to the given user-agent, in
// declaration order. The result contains no group/agent state and is safe to
// persist as minimized policy evidence.
func (r *RobotsData) Rules(agent string) []Rule {
	...
	for _, rl := range g.rules {
		ru := Rule{Path: rl.path, Allow: rl.allow}
		if rl.pattern != nil {
			ru.Pattern = rl.pattern.String()
		}
		out = append(out, ru)
	}
	return out
