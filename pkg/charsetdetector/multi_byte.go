		...
		if second >= 0x30 && second <= 0x39 {
			...
			if third >= 0x81 && third <= 0xFE {
				...
				if fourth >= 0x30 && fourth <= 0x39 {
					// Valid 4-byte GB18030. c already holds first<<8|second (>0xFF),
					// which is enough for multi-byte confidence scoring. Packing all
					// four bytes needs uint32 and is unused by commonChars lookup.
					return
				}
			}
		}
